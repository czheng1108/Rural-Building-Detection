            ...
            for i in range(crop_n):
                ...
                if crop_type == 'RANDOM_CROP':
                    crop_top_left_x, crop_top_left_y = random.randint(
                        0, imgw - crop_imgw - 1), random.randint(0, imgh - crop_imgh - 1)
                elif crop_type == 'CENTER_CROP':
                    crop_top_left_x, crop_top_left_y = int(
                        imgw / 2 - crop_imgw / 2), int(imgh / 2 - crop_imgh / 2)
                elif crop_type == 'FIVE_CROP':
                    if i == 0:
                ...
                croped_img_name = xml_name.split('.')[0] + '_' + \
                                  name_suffix + str(crop_top_left_x) + \
                                  '_' + str(crop_top_left_y) + '_wh' + str(crop_imgw) + \
                                  'x' + str(crop_imgh) + '.' + img_file.split('.')[-1]
                croped = crop_img_xml(
                    img,
    ...
    for root, dirs, files in os.walk(imgs_dir):
        for file in files:
            ...
            if count % 10 == 0:
                print(
                    '[%d|%d] %d%%' %
                    (count, imgcount, count * 100 / imgcount))
