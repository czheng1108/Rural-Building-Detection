...
def generate_img_xml_from_dir(imgs_dir,xmls_dir,imgs_save_dir,xmls_save_dir,name_suffix,\
    ...
    for n in range(count):
        print('Currently stitched {}th picture'.format(n))
        img_save_name  = name_suffix+'_'+str(n)+'.png'
...
def main():
    imgs_dir = 'data/JPEGImages'
    bk_imgs_dir = 'data/JPEGImages/'
    xmls_dir = 'data/Annotations/'
    
    imgs_save_dir= 'data/mosaic_imgs/'
    if not os.path.exists(imgs_save_dir):
        os.makedirs(imgs_save_dir)
    xmls_save_dir= 'data/mosaic_xmls/'
    if not os.path.exists(xmls_save_dir):
        os.makedirs(xmls_save_dir)
    
    name_suffix='mosaic' #命名标识
    count = 400 #拼接100张图片
    imgw,imgh = 1000,1000 #每张拼接图的大小
    part_imgw,part_imgh = int(imgw/2),int(imgh/2)
