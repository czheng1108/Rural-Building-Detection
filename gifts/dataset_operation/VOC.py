...
class VOC(object):
    ...
    def _area(self, write_txt):
        width = []
        height = []
        common = [0, 0, 0, 0]
        construction = [0, 0, 0, 0]
        common_area = [0, 0, 0]
        construction_area = [0, 0, 0]
        pa = self._ParseAnnos()
        for num in pa:
            for info in num['info']:
                w = info[3] - info[1]
                h = info[4] - info[2]
                width.append(w)
                height.append(h)
                if info[0] == 'Common Buildings':
                    if w/h <= 0.5:
                        common[0] = common[0] + 1
                    elif w/h <= 1:
                        common[1] = common[1] + 1
                    ...
                    if w*h <= 32*32:
                        common_area[0] = common_area[0] + 1
                    elif w*h <= 96*96:
                        common_area[1] = common_area[1] + 1
                    else:
                        common_area[2] = common_area[2] + 1
                if info[0] == 'Buildings under Construction':
                    if w/h <= 0.5:
                        construction[0] = construction[0] + 1
                    elif w/h <= 1:
                        construction[1] = construction[1] + 1
                    elif w/h <= 2:
                        construction[2] = construction[2] + 1
                    else:
                        construction[3] = construction[3] + 1

                    if w*h <= 32*32:
                        construction_area[0] = construction_area[0] + 1
                    elif w*h <= 96*96:
                        construction_area[1] = construction_area[1] + 1
                    else:
                        construction_area[2] = construction_area[2] + 1
        if write_txt:
            with open('w_h.txt', 'w') as f:
    ...
    def per_image_analysis(self):
        ...
        for num in pa:
            ...
            anchor_count.append(com + con)

        with open('per_image_analysis.csv', 'w') as f:
            for i in range(len(common)):
                f.write(str(common[i])
                        + ','
                        + str(construction[i])
                        + ','
                        + str(anchor_count[i])
...
if __name__ == '__main__':
    v = VOC('../../VOCdevkit/VOC2012/Annotations', '../../VOCdevkit/VOC2012/JPEGImages')
    # width, height = v._area(write_txt=True)
    v.per_image_analysis()
    # pa = v._ParseAnnos()
    # # print(pa)
    # count = v._Countobject()
    # print(count)