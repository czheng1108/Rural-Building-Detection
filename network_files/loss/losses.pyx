...
from network_files.loss import ghm_loss


def ghm_c_loss(inputs: torch.Tensor,
    ...
    """
    label_weight = torch.tensor(1.0, device=inputs.device)
    # label_weight = torch.sum(label_weight, dim=1)
    ghm_c = ghm_loss.GHMC_loss()
