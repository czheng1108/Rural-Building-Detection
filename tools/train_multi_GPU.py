...
from utils.train_utils import GroupedBatchSampler, create_aspect_ratio_groups, init_distributed_mode, save_on_master, mkdir
from config import configs


def create_model_use_trained_model(num_classes, device):
...
def create_model(num_classes, device):
    ...
    weights_dict = torch.load("./pre_training_weights/retinanet_resnet50_fpn.pth", map_location=device)
...
def main(args):
    ...
    # check voc root
    if os.path.exists(os.path.join(VOC_root, "../data/VOCdevkit")) is False:
    ...
    if configs.using_trained_weights:
        print('using \'{}\' to train weights!'.format(configs.resume.split('/')[-1]))
        model = create_model_use_trained_model(num_classes=args.num_classes + 1, device=device)
    else:
        print('using \'retinanet_resnet50_fpn.pth\' to train weights!')
        model = create_model(num_classes=args.num_classes + 1, device=device)
    model.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_image-only",
        dest="test_only",
        help="Only test_image the model",
        action="store_true",
    )
