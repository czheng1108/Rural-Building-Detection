...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True, norm_layer=None):
        ...
        if cfg.attention['fun'] == 'Attention':
            if 1 in cfg.attention['site']:
                self.att1 = Attention(256)
            self.layer2 = self._make_layer(block, 128, blocks_num[1], stride=2)
            if 2 in cfg.attention['site']:
                self.att2 = Attention(512)
            self.layer3 = self._make_layer(block, 256, blocks_num[2], stride=2)
            if 3 in cfg.attention['site']:
                self.att3 = Attention(1024)
        elif cfg.attention['fun'] == 'None':
            self.layer2 = self._make_layer(block, 128, blocks_num[1], stride=2)
            self.layer3 = self._make_layer(block, 256, blocks_num[2], stride=2)
        else:
